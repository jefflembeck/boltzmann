        ...
        if let Some(preconditions) = candidate.preconditions {
            ...
            if wants_feature {
                if !has_dep_currently {
                    info!("        adding {} @ {} ({} activated)", candidate.name.bold().magenta(), candidate.version, preconditions.all_of.join(", "));
                }
                target.insert(candidate.name, candidate.version.into());
            } else if wants_feature != used_to_have {
                if has_dep_currently {
                    info!("        removing {} ({} deactivated)", candidate.name.magenta().strikethrough(), preconditions.all_of.join(", "));
                }
